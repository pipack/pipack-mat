...
classdef ChebZeroNSG < NodeSetGenerator
    ...
    methods
        ...
        function this = ChebZeroNSG(ordering, precision, options)
    ...
    methods(Access = protected)
        ...
        function [z_j] = dbl_nodes(~, j, q, varargin)
            ...
            elseif(mod(q, 2) == 1)                
                ...
                j_left  = 1:qh/2;
                j_right = qh/2 + 1:qh;               
                z_all   = [-cos(pi*(j_left-1)/(qh-1)), 0, -cos(pi*(j_right-1)/(qh-1))];
                z_j     = z_all(j);
            else
        ...
        function [z_j] = vpa_nodes(~, j, q, varargin)
            ...
            elseif(mod(q, 2) == 1)                
                ...
                j_left  = 1:qh/2;
                j_right = qh/2 + 1:qh;               
                z_all   = [-cos(vpa('pi') * vpa(j_left-1) / vpa(qh-1)), 0, -cos(vpa('pi') * vpa(j_right-1) / vpa(qh-1))];
                z_j     = z_all(j);
            else
                error('The node set only supports odd number of nodes');
            end
        ...
        function [z_j] = sym_nodes(~, j, q, varargin)
            ...
            elseif(mod(q, 2) == 1)                
                ...
                j_left  = 1:qh/2;
                j_right = qh/2 + 1:qh;               
                z_all   = [-cos(sym('pi') * sym(j_left-1)/sym(qh-1)), 0, -cos(sym('pi') * sym(j_right-1) / sym(qh-1))];
                z_j     = z_all(j);
            else
                error('The node set only supports odd number of nodes');
            end
